...
# Manual prompt template definition (replaces hub.pull("langchain-ai/qa-system"))
...
class VectorStoreBase(ABC):
    """Abstract base class for vector stores to allow duck typing."""
    pass
...
class RAGChain:
    ...
    def __init__(
        ...
        self.collection_name = collection_name or os.getenv("CHROMA_COLLECTION_NAME", "rag_collection")
        ...
        self.temperature = temperature or float(os.getenv("LM_STUDIO_TEMPERATURE", "0.7"))
        self.max_tokens = max_tokens or int(os.getenv("LM_STUDIO_MAX_TOKENS", "1024"))
        self.persistent = persistent or (os.getenv("CHROMA_PERSISTENT", "false").lower() == "true")
    ...
    def load_and_store_document(
        ...
        metadatas = [{"source": str(file_path), "chunk_id": i} for i in range(len(chunks))]
